...
def test_classification_mlp_xor():
    ...
    data = pd.read_csv(SRC_DIR / "xor.data").set_index("index")
    ...
    for fold in range(1, K_FOLDS + 1):
        ...
        Y_tr = train.copy()[label].astype(np.float64).values
        Y_val = val.copy()[label].astype(np.float64).values
        Y_te = test.copy()[label].astype(np.float64).values
    ...
    # Save outputs
    print("\tSave")
    te_results_dst = DST_DIR / f"mlp_xor_te_results.csv"
    val_results_dst = DST_DIR / f"mlp_xor_val_results.csv"
    val_summary_dst = DST_DIR / f"mlp_xor_val_summary.csv"
